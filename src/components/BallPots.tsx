...
export const BallPots: React.FC<BallPotsProps> = ({
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-1.5 sm:space-y-2 relative">
      {/* Potted Balls in Current Break - Mini colored spheres with numbers and Ga badges */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl sm:rounded-2xl p-1 sm:p-2 shadow-md flex flex-wrap items-center justify-between gap-1 sm:gap-2">
        <div className="flex items-center space-x-1.5 text-[10px] sm:text-xs font-bold text-slate-300">
          <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span>ลูกที่ตบในเทิร์นนี้ ({pottedInVisit.length} ลูก):</span>
        </div>

        <div className="flex items-center space-x-1 sm:space-x-1.5 overflow-x-auto max-w-full py-0.5 px-0.5">
          {pottedInVisit.length === 0 ? (
            <span className="text-[10px] sm:text-xs text-slate-500 italic">ยังไม่มีลูกที่ตบลงในรอบนี้</span>
          ) : (
            pottedInVisit.map((s, idx) => {
              ...
              return (
                <div
                  key={s.id || idx}
                  className={`relative w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 rounded-full flex items-center justify-center font-mono font-black text-[10px] sm:text-xs text-white shadow-md border border-white/30 select-none ${b.cssClass}`}
                  title={`ลูก${b.nameTh} (+${b.points}${s.gaCount ? ` | +${s.gaCount} กา` : ''})`}
                >
                  <span className="drop-shadow-sm">{b.points}</span>
                  {s.gaCount && s.gaCount > 0 ? (
                    <span className="absolute -bottom-1 -right-1 bg-purple-600 text-white font-mono text-[7px] sm:text-[8px] font-black px-0.5 sm:px-1 rounded-full border border-purple-300 shadow">
                      +{s.gaCount}
                    </span>
                  ) : null}
                </div>
      ...
      <div className={`rounded-xl sm:rounded-2xl p-1.5 sm:p-2.5 md:p-3 shadow-lg transition-all border ${
        isFoulMode
        ...
        {isFoulMode && isGaMode ? (
          <div className="space-y-1.5 sm:space-y-2 py-0.5 animate-fadeIn">
            {/* Compact Ga deduction options */}
            <div className="bg-slate-950/80 p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl border border-rose-900/60 space-y-1 sm:space-y-1.5">
              <div className="flex items-center justify-between text-[10px] sm:text-xs text-slate-300 font-bold">
                <span className="flex items-center space-x-1">
                  <Target className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-purple-400" />
                  <span>เลือกจำนวนกาที่เสีย:</span>
                </span>
                <span className="text-purple-300 font-mono font-black text-[10px] sm:text-xs">
              ...
              <div className="grid grid-cols-4 sm:grid-cols-7 gap-1 sm:gap-1.5">
                {[0, 1, 2, 3, 4, 5, 6].map((ga) => (
                  <button
                    ...
                    className={`py-1.5 sm:py-2 px-1 rounded-lg sm:rounded-xl font-mono font-bold text-xs sm:text-sm border transition-all cursor-pointer ${
            ...
            <div className="flex space-x-1.5 sm:space-x-2 pt-0.5">
              <button
                type="button"
                onClick={onToggleFoulMode}
                className="flex-1 py-2 sm:py-2.5 rounded-lg sm:rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs sm:text-sm border border-slate-700 cursor-pointer"
              >
                ยกเลิก
              </button>
              <button
                ...
                className="flex-2 py-2 sm:py-2.5 rounded-lg sm:rounded-xl bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-500 hover:to-red-500 text-white font-black text-xs sm:text-sm border border-rose-400 shadow-lg shadow-rose-950/60 cursor-pointer active:scale-98 flex items-center justify-center space-x-1.5"
              >
            ...
            {ballList.map((ballKey) => {
              ...
              if (isFoulMode && !isFoulTarget) {
                return (
                  <div
                    key={ballKey}
                    className="invisible pointer-events-none py-2 sm:py-3 px-0.5"
                  />
                );
              }
              ...
              return (
                <button
                  ...
                  className={`group relative flex flex-col items-center justify-center py-2 sm:py-3 md:py-3.5 px-0.5 sm:px-1 rounded-xl sm:rounded-2xl transition-all duration-150 cursor-pointer border active:scale-92 ${ball.cssClass} hover:brightness-115 hover:shadow-lg shadow-md hover:-translate-y-0.5`}
                ...
                >
                  ...
                  <span className="font-mono font-black text-lg sm:text-2xl md:text-3xl lg:text-4xl leading-none drop-shadow-md select-none">
                    {isFoulMode ? `-${ball.points}` : ball.points}
                  </span>

                  {/* Keyboard Shortcut Hint Badge */}
                  <span className="absolute -top-1 -right-1 bg-slate-950/90 text-amber-300 text-[7px] sm:text-[9px] md:text-[10px] font-mono font-bold px-1 rounded-full border border-slate-700 shadow">
                    {ball.numpadKey}
                  </span>
                </button>
              );
      ...
      <div className="flex items-stretch gap-1 sm:gap-1.5 md:gap-2">
        ...
        <button
          onClick={onToggleFoulMode}
          className={`w-24 sm:w-32 md:w-40 flex-shrink-0 flex items-center justify-center space-x-1 sm:space-x-1.5 font-extrabold py-2 sm:py-2.5 md:py-3 px-1.5 sm:px-2.5 rounded-xl sm:rounded-2xl border transition-all cursor-pointer active:scale-98 shadow-md ${
            isFoulMode
        ...
        >
          <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-amber-300 flex-shrink-0 animate-bounce" />
          <div className="text-left min-w-0">
            <div className="text-xs sm:text-sm md:text-base leading-none font-black truncate">{isFoulMode ? 'ยกเลิก' : 'ฟาวล์'}</div>
            <div className="text-[8px] sm:text-[9px] text-rose-200 font-normal hidden xs:block mt-0.5">[-] / [F]</div>
          </div>
        </button>
        ...
        {isGaMode && (
          <button
            ...
            className="flex-1 flex flex-col items-center justify-center bg-gradient-to-r from-purple-700 to-indigo-800 hover:from-purple-600 hover:to-indigo-700 text-white font-extrabold py-1.5 sm:py-2 px-1 rounded-xl border border-purple-500 shadow-sm cursor-pointer active:scale-98 transition-all"
            title="เพิ่ม/ปรับจำนวนกาโดยตรง"
          >
            <Target className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-amber-300 flex-shrink-0" />
            <span className="text-[9px] sm:text-[11px] leading-tight font-black truncate mt-0.5">เพิ่มกา</span>
          </button>
        )}
        ...
        <button
          ...
          className="flex-1 flex flex-col items-center justify-center bg-slate-800/95 hover:bg-slate-700 text-slate-100 font-bold py-1.5 sm:py-2 px-1 rounded-xl border border-slate-700 shadow-sm cursor-pointer active:scale-98 transition-all"
        >
          <RotateCcw className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-amber-400 flex-shrink-0" />
          <span className="text-[9px] sm:text-[11px] leading-tight font-bold truncate mt-0.5">พลาด</span>
        </button>
        ...
        <button
          ...
          className="flex-1 flex flex-col items-center justify-center bg-slate-800/95 hover:bg-slate-700 text-slate-100 font-bold py-1.5 sm:py-2 px-1 rounded-xl border border-slate-700 shadow-sm cursor-pointer active:scale-98 transition-all"
        >
          <Shield className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-sky-400 flex-shrink-0" />
          <span className="text-[9px] sm:text-[11px] leading-tight font-bold truncate mt-0.5">กัน/เซฟ</span>
        </button>
        ...
        <button
          onClick={onUndo}
          disabled={!canUndo}
          className={`flex-1 flex flex-col items-center justify-center font-bold py-1.5 sm:py-2 px-1 rounded-xl border transition-all cursor-pointer ${
            canUndo
              ? 'bg-amber-600/90 hover:bg-amber-500 text-white border-amber-500 shadow-sm active:scale-98'
              : 'bg-slate-900 text-slate-600 border-slate-800 cursor-not-allowed'
          }`}
        >
          <Undo2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" />
          <span className="text-[9px] sm:text-[11px] leading-tight font-bold truncate mt-0.5">ย้อนกลับ</span>
        </button>

        {/* End Frame Button - Compact */}
        <button
          onClick={onEndFrame}
          className="flex-1 flex flex-col items-center justify-center bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-500 hover:to-yellow-500 text-slate-950 font-black py-1.5 sm:py-2 px-1 rounded-xl border border-amber-400 shadow-sm cursor-pointer active:scale-98 transition-all"
        >
          <Flag className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-950 flex-shrink-0" />
          <span className="text-[9px] sm:text-[11px] font-black leading-tight truncate mt-0.5">จบเฟรม</span>
        </button>

        {/* New Match Button Next to End Frame - Compact & Vibrant Neon */}
        <button
          onClick={onNewMatch}
          className="flex-1 flex flex-col items-center justify-center bg-gradient-to-r from-cyan-500 via-teal-500 to-emerald-500 hover:from-cyan-400 hover:to-emerald-400 text-slate-950 font-black py-1.5 sm:py-2 px-1 rounded-xl border-2 border-cyan-300 ring-1 ring-cyan-400/50 shadow-sm cursor-pointer active:scale-98 transition-all"
          title="เริ่มแมตช์ใหม่"
        >
          <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-950 flex-shrink-0 animate-pulse" />
          <span className="text-[9px] sm:text-[11px] font-black leading-tight truncate mt-0.5">เริ่มใหม่</span>
        </button>
      </div>
